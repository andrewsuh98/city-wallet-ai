...
async def get_offer_qr(offer_id: str, db=Depends(get_db_dep)):
...
async def validate(token: str, db=Depends(get_db_dep)):
...
async def redeem(req: RedemptionRequest, db=Depends(get_db_dep)):
...
async def get_wallet(session_id: str, db=Depends(get_db_dep)):
